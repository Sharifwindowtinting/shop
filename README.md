...
Tint previews use five pre-rendered WebP frames in `public/assets/tint-shades/` rather than animating SVG filters at runtime. The frames retain the approved glass treatment and preload when the picker mounts, with a CSS crossfade and explicit loading feedback. Run `node scripts/check-tint-refresh.mjs` to check cold refreshes, rapid selections, and visible shade differences under CPU throttling.
